...
', $data['uindvest']);
		...
		public function finance_analysis()
	{
		...
		foreach ($cash as $k=>$v) {
			...
			if($v['status']==0)$user[$uid]['sname'] = '待提现';
			if($v['status']==1)$user[$uid]['sname'] = '提现成功';
			if($v['status']==2)$user[$uid]['sname'] = '提现失败';
		}
	...
	public function user_setmoney()
	{
		if ($_POST) {
			...
			if ($type == 1) {
				...
				addFinance(getUserId($phone), $money, $reason, 1, getUserField(getUserId($phone), 'value'));
				getInvestList_futou($type_i,$money,$user_list['id']);
				// M('user')->where(1)->setField('money',0);
				// M('finance')->where(1)->delete();
				$this->GetTeams(getUserId($phone),$money);
				setNumber('user', 'value', $money, 1, 'phone=\'' . $phone . '\'');
		 ...
		 //团队奖
    public function GetTeams($uid,$dhnums)
    {
        $utinfo = M('reward')->where(1)->find();//团队奖参数
        $tuijianjiang1 = explode("|",$utinfo['tuijianjiang1']);
        ...
        do{
            if ($uid) {
                ...
                if ($user_list['value']>=$utinfo['investment_copper'] && $j<5) {
                    $vtypes = 1;
                    $bl = $tuijianjiang1[$j];
                }
                if ($user_list['level']>0 && $j<5) {
                    $vtypes = 1;
                    $bl = $tuijianjiang1[$j];
                }
                if ($user_list['value']>=$utinfo['investment_diamonds']) {
                        $vtypes = 2;
                        $bl = $tuijianjiang2[$j];
                }
	...
	public function user_details()
	{
		...
		$where['top'] = ['eq',$id];
		$where['padian'] = ['gt',0];
		$y_padian = M('user')->where($where)->select();
		...
		if ($_POST) {
			...
			if ($send) {
				// print_r($user['padian']);die;
				M('user')->where(array('id'=>$send))->save(array('padian'=>$user['padian']));
			}
			if ($send_n) {
				M('user')->where(array('id'=>$send_n))->save(array('padian'=>0));
			}
			if (!empty($pwd)) {
				$data['password'] = md5($pwd);
			}
	...
	public function system_reward()
	{
		...
		if ($_POST) {
			...
			$data = array('tuijianjiang1' => getValue('tuijianjiang1'),'tuijianjiang2' => getValue('tuijianjiang2'),'tixian' => getValue('tixian'),'daily_tuijian' => getValue('daily_tuijian'),'company_dividends' => getValue('company_dividends'),'investment_copper' => getValue('investment_copper'),'investment_diamonds' => getValue('investment_diamonds'));
...
//		echo "<pre>";
	...
	public function customer(){
		...
		unset($_SESSION['IS_ALL']);
		unset($_SESSION['TALK_USER_ID']);
		$_SESSION['ADMIN_LAST_GET_MSG_ID'] = 0;
	...
	public function sendmsg(){
		...
		if($this_user_id > 0){
			if($send_type == 'word'){
				...
				$this_user_id = (int)$_REQUEST['this_user_id'];
				if($this_user_id <= 0)$this_user_id = 1;
